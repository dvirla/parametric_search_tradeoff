...
| §`sec:epistemic_instrumentation`, §`sec:policy_not_uncertainty` — semantic entropy under perturbation | **DONE 2026-09-08.** The `no_search` probe is complete (6 models x 4 perturbations x 5 runs = 36,000 rollouts) and all 24 combos are clustered (gpt-oss:120b judge, same as FRAMES/MedQA). Entropy-vs-correctness, entropy-vs-search and entropy-under-cue all now cover HotpotQA. **See [`cross_dataset_uncertainty_findings.md`](cross_dataset_uncertainty_findings.md)** for the three-dataset synthesis, the EM-vs-judge attenuation caveat, and the revised MedQA reading. Still missing: `confident_parametric` was not among the 4 perturbations probed, so the belief-vs-policy status of the largest search effect is unmeasured on this dataset. |
