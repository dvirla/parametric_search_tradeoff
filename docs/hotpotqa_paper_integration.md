...
| §`sec:epistemic_instrumentation`, §`sec:policy_not_uncertainty` — semantic entropy under perturbation | **DONE 2026-09-08.** The `no_search` probe is complete (6 models x 4 perturbations x 5 runs = 36,000 rollouts) and all 24 combos are clustered (gpt-oss:120b judge, same as FRAMES/MedQA). Entropy-vs-correctness, entropy-vs-search and entropy-under-cue all now cover HotpotQA. **See [`cross_dataset_uncertainty_findings.md`](cross_dataset_uncertainty_findings.md)** for the three-dataset synthesis, the EM-vs-judge attenuation caveat, and the revised MedQA reading. **CLOSED 2026-09-11:** `confident_parametric` *was* subsequently probed and is now clustered for all 6 models (5/5 runs each; verified — every non-SFT model under `results/hotpotqa_parametric/` carries 5 cue cluster files). Result: Δentropy **+0.022 bits, 0 of 6 models significant**, against a +59.9pp zero-search shift. The HotpotQA uncertainty grid is complete at 3 datasets x 6 models x 4 cues = 72 cells. Full analysis, including the power/bootstrap bounding of the null and the in-domain-vs-pooled family question, is in [`cross_dataset_uncertainty_findings.md`](cross_dataset_uncertainty_findings.md) §3.0 — **that document, not this one, is the authority on the uncertainty axis.** Still genuinely unavailable on HotpotQA: LLM-judge grading, thinking-token/suppression data, and `cue_suppression_mechanism` rows. The SFT-checkpoint entropy arm is separate and partial (gemma4-frames-robust 2 of 5 cue files; gemma4-frames-resolved 5 of 5) and must not be folded into the 6-model roster. |
