...
Usage:
    uv run python scripts/analyze_modal_answer_shift.py
"""
import csv
import glob
import json
...
def majority_representative(cluster_ids, responses):
    ...
    (deterministic). None if no majority (all 3 runs in distinct clusters)."""
...
def main():
    ...
    for ds, cfg in DATASETS.items():
        for model, tag in TAGS.items():
            ...
            plain_cluster_path = os.path.join(model_dir, f"{cfg['prefix']}_no_search_{tag}_llm_clusters.json")
            if not os.path.exists(plain_cluster_path):
                continue
            clusters_plain = load_clusters(plain_cluster_path)
            responses_plain = load_responses(model_dir, cfg["prefix"], tag, None)
            if responses_plain is None:
                continue

            cue_cluster_paths = glob.glob(os.path.join(model_dir, f"{cfg['prefix']}_no_search_{tag}_*_llm_clusters.json"))
            for cue_path in sorted(cue_cluster_paths):
                ...
                responses_cue = load_responses(model_dir, cfg["prefix"], tag, cue)
                ...
                consensus_breakdown = 0   # plain had a majority (1 or 2 clusters), cue is a 3-way tie
                consensus_formed = 0      # plain was a 3-way tie, cue has a majority
                n_eligible = 0
                ...
                for eid in common:
                    ...
                    if n_plain in (1, 2) and n_cue == 3:
                        consensus_breakdown += 1
                    if n_plain == 3 and n_cue in (1, 2):
                        consensus_formed += 1
    ...
    trans_path = os.path.join(OUT_DIR, "cluster_count_transitions.csv")
    ...
    print("=== cluster-count (consensus structure) transitions, plain -> cue ===")
    print("n_clusters: 1 = all 3 runs agree, 2 = majority of 2, 3 = three-way tie (no modal answer)\n")
    for r in sorted(transition_rows, key=lambda r: (r["dataset"], r["model"], r["cue"])):
        ...
        print(f"  {r['dataset']:6s} {r['model']:20s} {r['cue']:22s} n={r['n']:4d}  "
              ...
              f"breakdown(majority->3way): {r['consensus_breakdown']} ({r['pct_consensus_breakdown']}%)  "
              f"formed(3way->majority): {r['consensus_formed']} ({r['pct_consensus_formed']}%){flag}")
    ...
    print(f"Total consensus-breakdown examples (majority -> 3-way tie) across all cells: {total_breakdown}.")
