...
For each open-ended question, uses an LLM to identify 3–5 specific,
objectively-verifiable factual claims the world must make true for any
well-grounded answer. These serve as "hops" for the parametric uncertainty
evaluation pipeline (run_sharechat_parametric_uncertainty.py).
...
Given a question, produce 3–5 short, specific probe questions about objectively-verifiable \
facts that the world must make true for any well-grounded answer to the original question.
...
4. Keep probes short and self-contained. No "Does this mean..." or "Given that..." phrasing.
5. Return ONLY a valid JSON array of strings. No explanation, no preamble.\
"""
...
class SubstrateQuestions(BaseModel):
    questions: list[str] = Field(
        description="List of factual substrate probe questions.",
        min_length=1,
        max_length=6,
    )
...
async def label_one(agent: BaseAgent, question: str, num_substrates: int) -> list[str]:
    prompt = SUBSTRATE_TEMPLATE.format(question=question)
    response = await agent.arun(prompt)
    result: SubstrateQuestions = response.output
    return result.questions[:num_substrates]
...
async def run(args: argparse.Namespace) -> None:
    ...
    with open(args.output, "a") as out_f:
        for i, question in enumerate(pending):
            ...
            try:
                substrate_questions = await label_one(agent, question, args.num_substrates)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-substrates", type=int, default=4,
                        help="Max substrate questions per query (default: 4).")
